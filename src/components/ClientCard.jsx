...
function ClientCard({ client }) {
  return (
    <Link
      to={`/client/${client.id}`}
      className="block bg-white rounded-xl shadow border border-slate-200 p-4 hover:border-slate-400 transition"
    >
      ...
      <p className="text-xs text-slate-400 mt-2 line-clamp-2">{client.projectScope}</p>
    </Link>
  )
}
